...
printf '%s\n' '[1/11] Compile current Python sources'
...
printf '%s\n' '[2/11] Run the V29.18 pytest validator suite'
"${PYTHON_BIN}" -m pytest -q

printf '%s\n' '[3/11] Verify modular architecture dependencies'
"${PYTHON_BIN}" tools/test_dependency_analyser.py

printf '%s\n' '[4/11] Run independent-process deterministic smoke checks'
...
printf '%s\n' '[5/11] Run the current V29 simulator and certify its summary'
mkdir -- "${TEMP_DIR}/main"
(
    cd -- "${TEMP_DIR}/main"
...
printf '%s\n' '[6/11] Verify the complete public manifest'
"${PYTHON_BIN}" tools/check_manifest.py

printf '%s\n' '[7/11] Generate and compare the Rust oracle reference'
...
printf '%s\n' '[11/11] Verify validation did not modify tracked files'
